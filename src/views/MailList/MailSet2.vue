  ...
  <div class="MailList">
    ...
    <Drawer width="900" :closable="false" v-model="isShowDrawer">
      <div slot="header" class="header-drawer">
        <div><Icon class="mr-10" color="#2D8CF0" type="md-create" />测试专用</div>
        <div>
  ...
  methods: {
    ...
    async createFansTask() {
      ...
      if (flag) {
        ...
        const ms = await this.$http.post("/contact/addPhoneToContact", upload)
        this.$Message.info(`通讯录上传下载共耗时${ms}`)
        const { msg } = await this.$http.post("/contact/addFriendsByContactBatch", args)
